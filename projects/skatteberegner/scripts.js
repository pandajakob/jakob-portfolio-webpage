...
function get_kommune_data(kommune) {
  return skat_data["kommuneskat"].find((k) => {
    return k.Kommune == kommune;
  });
}

let skat_data = [];

let dkk = new Intl.NumberFormat("da", {
  style: "currency",
  currency: "DKK",
}).format(1234.56);
...
function calculate_tax(e) {
  ...
  // fjern personfradrag
  const brugt_personfradrag =
    skat_data.personfradrag < indkomst_state
      ? skat_data.personfradrag
      : indkomst_state;
  ...
  if (indkomst_state <= skat_data.topskat_grnse) {
    ...
    calc_tax_data["topskat"] =
      (indkomst_state - skat_data.topskat_grnse) * topskat;
    calc_tax_data["bundskat"] = indkomst_state * bundskat;
    calc_tax_data["kommuneskat"] = indkomst_state * kommuneskat;
    calc_tax_data["kirkeskat"] = skat_data.topskat_grnse * kirkeskat;
    indkomst_state -=
      calc_tax_data["topskat"] +
      calc_tax_data["bundskat"] +
      calc_tax_data["kommuneskat"] +
      calc_tax_data["kirkeskat"];
  }
